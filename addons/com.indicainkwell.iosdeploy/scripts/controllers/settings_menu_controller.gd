...
func _on_about_to_show():
	var v = get_view()
	v.remote_debug = stc.get_config().get_value('deploy', 'remote_debug', false)
	v.ios_deploy_path = stc.get_config().get_value('deploy', 'ios_deploy_path', stc.DEFAULT_IOSDEPLOY_TOOL_PATH)
...
func _on_hide():
	var v = get_view()
	stc.get_config().set_value('deploy', 'remote_debug', v.remote_debug)
	stc.get_config().set_value('deploy', 'ios_deploy_path', v.ios_deploy_path)
